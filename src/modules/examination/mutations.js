export default {
  createExamination: async (parent, args, context) => {
    ...
    const { subject, description, maximumMarks, time, maxAttempts } = args;
    const response = await examinationApi.create({ subject, description, maximumMarks, time, maxAttempts });
    return response;
  },
